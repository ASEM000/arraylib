...
static inline f32 add32(f32 lhs, f32 rhs) { return lhs + rhs; }
...
NDArray* array_empty(size_t* shape, size_t ndim) {
    ...
    array->stride = stride_from_shape(size_t_create(ndim), shape, ndim);
...
NDIterator iterator_create(f32* ptr, size_t* shape, size_t* stride, size_t* dims, size_t ndim) {
    ...
    iter.bstride = bstride_from_shape(size_t_create(ndim), shape, stride, ndim);
...
NDIterator array_iter(NDArray* array) {
    size_t* dims = size_t_create(array->ndim);
    size_t_set(dims, ITERDIM, array->ndim);
...
NDArray* array_deep_copy(NDArray* array) {
    ...
    NDIterator dst_iter = array_iter(out_array);
    NDIterator src_iter = array_iter(array);
...
f32 array_get_scalar_from_index(NDArray* array, size_t* index) {
    size_t findex = flat_index(index, array->stride, array->ndim);
...
NDArray* array_set_scalar_from_range(
        NDArray* array, size_t* start, size_t* end, size_t* step, f32 value
) {
    for (size_t i = 0; i < array->ndim; i++) {
    ...
    f32* ptr = &array->data->mem[array->offset];
    ptr += flat_index(start, array->stride, array->ndim);
...
NDArray* array_set_view_from_array(
        NDArray* array, size_t* start, size_t* end, size_t* step, NDArray* value
) {
    ...
    f32* ptr = &array->data->mem[array->offset];
    ptr += flat_index(start, array->stride, array->ndim);
    size_t* dims = size_t_set(size_t_create(ndim), ITERDIM,
                              ndim);  // iterate all
    NDIterator src = iterator_create(ptr, view_shape, view_stride, dims, ndim);
    NDIterator dst = array_iter(value);
...
NDArray* array_reshape(NDArray* array, size_t* shape, size_t ndim) {
    ...
    out_array->stride = stride_from_shape(size_t_create(ndim), shape, ndim);
    out_array->ndim = ndim;
    return out_array;
}
...
NDArray* array_scalar_op(NDArray* array, f32 rhs, binop fn) {
    NDArray* out_array = array_empty(array->shape, array->ndim);
    NDIterator src = array_iter(array);
    NDIterator dst = array_iter(out_array);
    while (iterator_iterate(&src) && iterator_iterate(&dst))
...
NDArray* array_scalar_add(NDArray* lhs, f32 rhs) { return array_scalar_op(lhs, rhs, add32); }
...
static inline void block_matmul(
        NDArray* dst,
        NDArray* lhs,
        NDArray* rhs,
        size_t i0,
...
NDArray* array_array_matmul(NDArray* lhs, NDArray* rhs) {
    ...
    // TODO: check SIMD compat with the
    // HW
    ...
    for (size_t i0 = 0; i0 < M; i0 += BLOCK_SIZE) {
        for (size_t j0 = 0; j0 < N; j0 += BLOCK_SIZE) {
            for (size_t k0 = 0; k0 < K; k0 += BLOCK_SIZE) {
                ...
                block_matmul(out_array, lhs, rhs, i0, j0, k0, imax, jmax, kmax);
            }
        }
    }
...
NDArray* array_array_scalar_op(NDArray* lhs, NDArray* rhs, binop fn) {
    ...
    NDIterator lhs_iter = array_iter(lhs);
    NDIterator rhs_iter = array_iter(rhs);
    NDIterator out_iter = array_iter(out);

    while (iterator_iterate(&lhs_iter) && iterator_iterate(&rhs_iter) && iterator_iterate(&out_iter)
    )
...
NDArray* array_array_dot(NDArray* lhs, NDArray* rhs) {
    ...
    while (iterator_iterate(&lhs_iter) && iterator_iterate(&rhs_iter))
        total = add32(total, mul32(*lhs_iter.ptr, *rhs_iter.ptr));
    iterator_free(&lhs_iter);
    iterator_free(&rhs_iter);
    NDArray* out_array = array_empty((size_t[]){1}, 1);
    array_set_scalar_from_index(out_array, (size_t[]){0}, total);
    return out_array;
}
